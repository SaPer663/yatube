...
class TestPost:
    ...
    def test_post_model(self):
        ...
        assert pub_date_field is not None, (
            'Добавьте дату и время проведения события в '
            f'`{pub_date_field_name}` модели `Post`'
        )
        ...
        assert image_field.upload_to == 'posts/', (
            "Свойство `image` модели `Post` должно быть "
            "с атрибутом `upload_to='posts/'`"
        )
    ...
    def test_post_admin(self):
        ...
        assert (
            ...
            'Добавьте `pub_date` или `created` для фильтрации '
...
class TestGroup:
    ...
    def test_group_model(self):
        ...
        assert type(title_field) == fields.CharField, (
            'Свойство `title` модели `Group` должно быть '
            'символьным `CharField`'
        )
...
class TestGroupView:
    ...
    def test_group_view(self, client, post_with_group):
        ...
        assert page_context is not None, (
            'Проверьте, что передали автора статьи в контекст страницы '
            f'`{url_templ}` типа `Page`'
        )
        assert len(page_context.object_list) == 1, (
            'Проверьте, что в контекст страницы переданы правильные '
            f'статьи автора `{url_templ}`'
        )
