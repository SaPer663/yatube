...
class TestPostView:
    ...
    def test_index_post_with_image(self, client, post):
        ...
        assert page_context is not None, (
            ...
            'главной страницы `/` типа `Page`'
        )
        assert len(page_context.object_list) == 1, (
            'Проверьте, что в контекст главной страницы переданы '
            'правильные статьи автора'
        )
    ...
    def test_post_view_get(self, client, post_with_group):
        try:
            ...
            assert False, (
                'Страница `/posts/<post_id>/` работает неправильно. '
                f'Ошибка: `{e}`'
            )
        ...
        assert type(
            comment_form_context.fields['text']
...
class TestPostEditView:
    ...
    def test_post_edit_view_get(self, client, post_with_group):
        try:
            ...
            assert False, (
                'Страница `/posts/<post_id>/edit/` работает неправильно. '
                f'Ошибка: `{e}`'
            )
        if (
    ...
    def test_post_edit_view_author_get(self, user_client, post_with_group):
        ...
        assert len(response.context['form'].fields) == fields_cnt, (
            ...
            f'`/posts/<post_id>/edit/` {fields_cnt} поля'
        )
        assert 'group' in response.context['form'].fields, (
            'Проверьте, что в форме `form` на странице '
            '`/posts/<post_id>/edit/` есть поле `group`'
        )
        assert type(
            response.context['form'].fields['group']
        ) == forms.models.ModelChoiceField, (
            'Проверьте, что в форме `form` на странице '
            '`/posts/<post_id>/edit/` поле `group` типа `ModelChoiceField`'
        )
        ...
        assert type(
            ...
            'Проверьте, что в форме `form` на странице '
            '`/posts/<post_id>/edit/` поле `text` типа `CharField`'
        )
        assert response.context['form'].fields['text'].required, (
            'Проверьте, что в форме `form` на странице '
            '`/posts/<post_id>/edit/` поле `group` обязательно'
        )

        assert 'image' in response.context['form'].fields, (
            'Проверьте, что в форме `form` на странице '
            '`/posts/<post_id>/edit/` есть поле `image`'
        )
        assert type(
            response.context['form'].fields['image']
        ) == forms.fields.ImageField, (
            'Проверьте, что в форме `form` на странице '
            '`/posts/<post_id>/edit/` поле `image` типа `ImageField`'
        )
    ...
    def test_post_edit_view_author_post(
        ...
        assert post is not None, (
            'Проверьте, что вы изменили пост при отправки формы на странице '
            '`/posts/<post_id>/edit/`'
        )
