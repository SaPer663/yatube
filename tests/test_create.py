...
class TestCreateView:
    ...
    def test_create_view_get(self, user_client):
        ...
        assert len(response.context['form'].fields) == fields_cnt, (
            'Проверьте, что в форме `form` на страницу `/create/`'
            f' {fields_cnt} поля'
        )
    ...
    def test_create_view_post(self, mock_media, user_client, user, group):
        ...
        assert post is not None, (
            'Проверьте, что вы сохранили новый пост при отправки формы '
            'на странице `/create/`'
        )
        assert response.url == f'/profile/{user.username}/', (
            'Проверьте, что перенаправляете на страницу профиля автора '
            f'`/profile/{user.username}`'
        )
        ...
        assert post is not None, (
            'Проверьте, что вы сохранили новый пост при отправке формы '
            'на странице `/create/`'
        )
        assert response.url == f'/profile/{user.username}/', (
            'Проверьте, что перенаправляете на страницу профиля автора '
