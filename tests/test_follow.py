...
class TestFollow:
    ...
    def test_follow(self):
        ...
        assert type(user_field) == fields.related.ForeignKey, (
            'Свойство `user` модели `Follow` должно быть ссылкой '
        ...
        assert author_field.remote_field.related_name == 'following', (
            'Свойство `author` модели `Follow` должно иметь аттрибут '
    ...
    def test_follow_not_auth(self, client, user):
        ...
        if not(response.status_code in (301, 302)
            ...
            assert False, (
                'Проверьте, что не авторизованного пользователя `/follow/` '
    ...
    def test_follow_auth(self, user_client, user, post):
        ...
        assert 'page_obj' in response.context, (
            ...
            'в контекст страницы `/follow/`'
        )
        assert type(response.context['page_obj']) == Page, (
            'Проверьте, что переменная `page_obj` на странице `/follow/` '
            'типа `Page`'
        )
