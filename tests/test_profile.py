...
class TestProfileView:
    ...
    def test_profile_view_get(self, client, post_with_group):
        ...
        try:
            ...
            assert False, (
                f'Страница `{url_templ}` работает неправильно. '
                f'Ошибка: `{e}`'
            )
        ...
        assert page_context is not None, (
            'Проверьте, что передали статьи автора в контекст страницы '
            f'`{url_templ}` типа `Page`'
        )
        assert len(page_context.object_list) == 1, (
            'Проверьте, что в контекст страницы переданы правильные статьи '
            f'автора `{url_templ}`'
        )
        ...
        assert len(page_context.object_list) == 0, (
            'Проверьте, что правильные статьи автора в контекст страницы '
            f'`{url_templ}`'
        )
