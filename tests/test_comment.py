...
class TestComment:
    ...
    def test_comment_model(self):
        ...
        assert pub_date_field is not None, (
            f'Добавьте дату и время проведения события `{pub_date_field_name}`'
            ' модели `Comment`'
        )
        assert type(pub_date_field) == fields.DateTimeField, (
            f'Свойство `{pub_date_field_name}` модели `Comment` должно быть'
            ' датой и время `DateTimeField`'
        )
        ...
        assert author_field is not None, (
            ...
            ' `author` модели `Comment`'
        )
        assert type(author_field) == fields.related.ForeignKey, (
            'Свойство `author` модели `Comment` должно быть ссылкой'
        ...
        assert type(post_field) == fields.related.ForeignKey, (
            'Свойство `group` модели `Comment` должно быть ссылкой'
            ' на другую модель `ForeignKey`'
        )
    ...
    def test_comment_add_view(self, client, post):
        try:
            ...
            assert False, (
                f'Страница `/posts/<post_id>/comment/` работает неправильно.'
                f' Ошибка: `{e}`'
            )
        ...
        assert response.status_code != 404, (
            'Страница `/posts/<post_id>/comment/` не найдена,'
    ...
    def test_comment_add_auth_view(self, user_client, post):
        ...
        assert comment is not None, (
            'Проверьте, что вы создаёте новый комментарий'
            ' `/posts/<post_id>/comment/`'
        )
